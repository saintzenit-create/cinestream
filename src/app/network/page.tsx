...
import { networkOriginals } from '@/data/content';

export default function NetworkPage() {
const networks = Array.from(
new Set(networkOriginals.map((item) => item.badge).filter(Boolean))
);
...
Browse by Network </h1>

```
    <div className="space-y-12">
      {networks.map((network) => {
        const items = networkOriginals.filter(
          (item) => item.badge === network
        );
